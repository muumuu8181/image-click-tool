#!/usr/bin/env python3
"""
画像クリックツール GUI版 v0.13
改行修正、ワークフロータブ内で全作業完結
"""
...
class ImageClickerGUIv013:
    """メインGUIアプリケーション v0.13"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("画像クリックツール v0.13")
    ...
    def setup_workflow_tab(self, parent):
        ...
        info_text.insert(tk.END, "【例】Google検索の自動化\n")
        info_text.insert(tk.END, "　　1) 検索ボックスをスクショ→クリック設定\n")
        info_text.insert(tk.END, "　　2) 待機2秒 3) 検索ボタンをスクショ→クリック設定 4) 実行で自動化！")
        info_text.config(state=tk.DISABLED)
        
        # 操作手順
        steps_frame = ttk.LabelFrame(parent, text="📝 操作手順（このタブ内で完結）", padding="10")
        steps_frame.pack(fill=tk.X, padx=5, pady=5)
        
        steps_text = tk.Text(steps_frame, height=5, wrap=tk.WORD, background="#ffffcc")
        steps_text.pack(fill=tk.X)
        steps_text.insert(tk.END, "【STEP1】 記録開始をクリック\n")
        steps_text.insert(tk.END, "【STEP2】 「スクショ撮影」で画像を保存\n")
        steps_text.insert(tk.END, "【STEP3】 「クリック追加」で保存した画像を選択\n")
        steps_text.insert(tk.END, "【STEP4】 「待機追加」で待機時間を設定\n")
        steps_text.insert(tk.END, "【STEP5】 STEP2-4を繰り返して、記録停止→実行！")
        ...
        ttk.Button(
            control_frame,
            text="📸 スクショ撮影",
            command=self.workflow_take_screenshot,
            width=15
        ...
        # ワークフロー用画像リスト
        workflow_images_frame = ttk.LabelFrame(parent, text="📷 ワークフロー用画像", padding="10")
        workflow_images_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # 2列レイアウト
        left_col = ttk.Frame(workflow_images_frame)
        left_col.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        right_col = ttk.Frame(workflow_images_frame)
        right_col.pack(side=tk.RIGHT, fill=tk.Y)
        
        # ワークフロー画像リストボックス
        workflow_scrollbar = ttk.Scrollbar(right_col)
        workflow_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.workflow_listbox = tk.Listbox(left_col, yscrollcommand=workflow_scrollbar.set, height=6)
        self.workflow_listbox.pack(fill=tk.BOTH, expand=True)
        workflow_scrollbar.config(command=self.workflow_listbox.yview)
        
        # リスト操作ボタン
        workflow_buttons = ttk.Frame(workflow_images_frame)
        workflow_buttons.pack(fill=tk.X, pady=2)
        
        ttk.Button(
            workflow_buttons,
            text="🔄 更新",
            command=self.refresh_workflow_images,
            width=10
        ).pack(side=tk.LEFT, padx=2)
    ...
    def workflow_take_screenshot(self):
        """ワークフロー用スクリーンショット撮影"""
        ...
        if selection:
            ...
            self.workflow_text.insert(tk.END, f"[{self.recorder.current_step-1}] 📸 撮影: {filename}\n")
            self.refresh_workflow_images()
    ...
    def toggle_recording(self):
        ...
        if not self.recorder.is_recording:
            ...
            self.workflow_text.insert(tk.END, "次は「📸 スクショ撮影」をクリックして画像を撮影してください\n")
            self.status_var.set("🔴 ワークフロー記録中... 📸スクショ撮影をクリックしてください")
        else:
    ...
    def add_workflow_click(self):
        ...
        if not selection:
            messagebox.showwarning("警告", "クリックする画像をワークフロー用画像リストから選択してください")
            return
        
        image_name = self.workflow_listbox.get(selection[0])
    ...
    def refresh_workflow_images(self):
        ...
        if self.clicker.images_dir.exists():
            self.workflow_images = list(self.clicker.images_dir.glob("*.png"))
            self.workflow_images.extend(list(self.clicker.images_dir.glob("*.jpg")))
            self.workflow_images.extend(list(self.clicker.images_dir.glob("*.jpeg")))
            
            for image_file in sorted(self.workflow_images):
                self.workflow_listbox.insert(tk.END, image_file.name)
